...
class Fitter:
    ...
    case, lambda = list1_conc0, list1_conc1 and z = list1_z = list1_angs/pi.
    The angles in list1_angs are in the interval [0, pi] so the entries 
    list1_z are in the interval [0, 1]. 
    ...
    def get_delbo_and_grad_delbo(self, list1_z, list1_conc0, list1_conc1):
        ...
        list1_angs = [list1_z[k]*np.pi for k in range(len1)]
        # log_py is a constant with shape 1
        log_py = np.sum(np.array(
            [self.mod.prob_y_for_given_x_and_angs_prior(y_nsam[sam],
        ...
        list1_delbo = [log_py + list1_log_pz[k] - list1_log_qz[k]
                 for k in range(len1)]
    ...
    def print_fit_values_at_fin_t(self):
        ...
        Recall z = ang/pi with ang in interval [0, pi] so z in interval [0,
        1].This function calculates for each z, its estimate, the std of
        that estimate, and the fractional error (z_estimate -
        z_prior)/z_prior. z_prior = angs_prior/pi.

        angs_prior are the prior angles assumed for the model. If we use
        training data generated by Model:get_toy_data( ), angs_prior are
        true values, the ones used to generate the synthetic data.
